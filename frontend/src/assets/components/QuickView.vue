<template>
    ...
    <div v-if="user" class="quick-view">
        <div class="quick-view-inner" v-for="f in selectedtire" :key="f">
            <h2 class="d-flex justify-content-between">{{ f.tire_name }}
                <slot></slot>
            </h2>
            <div class="product-detail d-flex">
                ...
                <div class="content">
                    <p class="desc">{{ f.tire_desc }}</p>
                    <p class="money">${{ parseFloat(f.tire_price) - parseFloat(f.tire_discount) }}<span
                            v-if="parseFloat(f.tire_discount) > 0">${{
...
export default {
    ...
    computed: {
        ...
        selectedtire: function () {
            return this.alltires.filter((f) => parseInt(f.tire_id) == parseInt(this.tire));
        }
    },
    ...
    methods: {
        ...
        async addToCart() {
            let existItem = await axios.get('/cartItem/' + parseInt(this.user.user_id) + '/' + parseInt(this.tire));

            if (existItem.data.length == 1) {
                let data = {
                    user_id: parseInt(this.user.user_id),
                    tire_id: parseInt(this.tire),
